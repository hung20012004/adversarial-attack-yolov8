...
class ExperimentRunner:
    ...
    def run_single_experiment(self, epsilon, alpha, exp_index):
        ...
        cmd = [
            sys.executable, 'atk.py',
            '--weights', self.base_config['weights'],
            '--data', self.base_config['data'],
            '--img-size', str(self.base_config['img_size']),
            '--batch-size', '1',
            '--attack_imgs', str(self.base_config['attack_imgs']),
            '--conf-thres', str(self.base_config['conf_thres']),
            '--iou-thres', str(self.base_config['iou_thres']),
            '--iter-atk', str(self.base_config['iter_atk']),
            '--attack_freq', self.base_config['attack_freq'],
            '--target_map_reduction', str(self.base_config['target_map_reduction']),
            '--epsilon', str(epsilon),
            '--alpha', str(alpha),
            '--attack',
            '--project', str(exp_dir),
