...
def create_frequency_mask(H, W, freq_type, r_h=0.2, r_w=0.2, d_min=0.2, d_max=0.6, device="cpu"):
    ...
    if freq_type == "low":
        ...
        raise ValueError("freq_type must be 'low', 'mid', 'high', or 'all'")
...
class DCT4OD_mAP:
    ...
    def _extract_predictions(self, model_output):
        """Extract predictions tensor from model output YOLOv8 can return: tuple, dict, or tensor.
        """
    ...
    def compute_map(self, predictions, targets, img_tensor, shapes):
        ...
        try:
            ...
            output = non_max_suppression(predictions.clone(), self.conf_thres, self.iou_thres, multi_label=True)
            ...
            for si, pred in enumerate(output):
                ...
                stats.append((correct.cpu().detach(), pred[:, 4].cpu().detach(), pred[:, 5].cpu().detach(), tcls))
        ...
        for i in pbar:
            ...
            if i % check_interval == 0 or i == self.max_iter - 1:
                ...
                with torch.no_grad():
                    ...
                    pbar.set_postfix(
                        {"mAP": f"{curr_map:.4f}", "best": f"{best_map:.4f}", "target": f"{target_map:.4f}"}
                    )
