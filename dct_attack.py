...
def set_seed(seed=0):
    """Thiết lập seed ngẫu nhiên cho tất cả thư viện để đảm bảo kết quả lặp lại được Mục đích: - Đảm bảo mỗi lần chạy
    với cùng than số sẽ cho kết quả giống hệt nhau Than số: seed (số nguyên): Giá trị hạt giống, mặc định là 0.
    """
...
def xywh2xyxy(x):
    """Chuyển đổi định dạng hộp giới hạn từ [tâm_x, tâm_y, rộng, cao] sang [x1, y1, x2, y2].
...
def scale_boxes(img1_shape, boxes, img0_shape, ratio_pad=None):
    """Thu phóng tọa độ box giới hạn từ kích thước ảnh đã thay đổi (ảnh1) về kích thước ảnh gốc (ảnh0).
...
def ap_per_class(tp, conf, pred_cls, target_cls, plot=False, save_dir=None, names=(), eps=1e-16):
    ...
    px, _py = np.linspace(0, 1, 1000), []  # 1000 điểm hồi tưởng để nội suy
    ...
    for ci, c in enumerate(unique_classes):
        ...
        for j in range(tp.shape[1]):
            ap[ci, j], _mpre, _mrec = compute_ap(recall[:, j], precision[:, j])
...
def create_frequency_mask(H, W, freq_type, r_h=0.2, r_w=0.2, d_min=0.2, d_max=0.6, device="gpu"):
    """Tạo mặt nạ: - Tần số thấp: Thông tin tổng thể, màu sắc, hình dạng lớn - Tần số tb: Chi tiết vừa phải, cạnh, kết
    cấu - Tần số cao: Chi tiết mịn, nhiễu, cạnh sắc nét.
    ...
    if freq_type == "low":
        ...
        raise ValueError("freq_type phải là 'low', 'mid', 'high', hoặc 'all'")
...
class DCT4OD_mAP:
    """1. Chuyển ảnh sang miền tần số (miền biến đổi cosin) 2. Thêm nhiễu vào các thành phần tần số được chọn
    (thấp/giữa/cao) 3. Chuyển ngược về miền không gian 4. Giữ nhiễu trong ngưỡng epsilon để không nhìn thấy bằng
    mắt thường.
    """
    ...
    def __init__(
        ...
        """Khởi tạo bộ tấn công Than số: model: Mô hình YOLOv8 epsilon: Ngưỡng nhiễu tối đa (0-1 hoặc 0-255) chuẩn hóa
        về 0-1 Vd: epsilon=8/255 = 0.031 max_iter: Số vòng lặp tối đa step_size: Kích thước bước mỗi vòng lặp
        (0-1 hoặc 0-255) target_map_reduction: Mức giảm độ chính xác mong muốn (0-1) Vd: 0.3 = giảm 30%
        conf_thres: Ngưỡng tin cậy cho lọc hộp iou_thres: Ngưỡng giao cho lọc hộp img_size: Kích thước ảnh đầu
        vào names: mảng mã_lớp - tên_lớp freq_type: Loại tần số tấn công ('low', 'mid', 'high', 'all') r_h, r_w:
        Tỷ lệ vùng tần số thấp d_min, d_max: Khoảng cách cho tần số tb.
        """
        self.model = model
        ...
        print(f"\n{'=' * 80}")
        print("Initializing Full-DCT Attack (YOLOv8)")
        ...
        self.freq_mask = create_frequency_mask(
            H=img_size, W=img_size, freq_type=freq_type, r_h=r_h, r_w=r_w, d_min=d_min, d_max=d_max, device=self.device
        )
    ...
    def _create_dct_matrix(self, n):
        """Tạo ma trận biến đổi cosin rời rạc dùng để chuyển ảnh sang miền tần số Ma trận này có kích thước n*n lưu vào
        bộ nhớ đệm để tái sử dụng.
        ...
        dct_m = torch.where(
            ...
            torch.sqrt(torch.tensor(2.0 / n, dtype=self.dtype)) * torch.cos(torch.pi * k * (2 * i + 1) / (2 * n)),
        )
    ...
    def compute_map(self, predictions, targets, img_tensor, shapes):
        ...
        try:
            ...
            if len(stats) and stats[0][0].any():
                ...
                _tp, _fp, _p, _r, _f1, ap, _ap_class = ap_per_class(*stats_np, plot=False, names=self.names)
    ...
    def run(self, original_image, targets, shapes, image_path, save_dir):
        ...
        """
        start_time = time.time()
        _nb, _, height, width = original_image.shape
        ...
        for i in pbar:
            ...
            if i % check_interval == 0 or i == self.max_iter - 1:
                ...
                with torch.no_grad():
                    ...
                    curr_metrics = self.compute_map(curr_preds, targets_scaled, adversarial_image.detach(), shapes)
